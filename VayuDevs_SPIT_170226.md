...
> **Team:** VayuDevs | **Campus:** Sardar Patel Institute of Technology | **Date:** 16/02/2026  
...
Financial institutions must file Suspicious Activity Reports (SARs) for every detected instance of money laundering, fraud, or financial crime. Manual SAR drafting averages 5–6 hours per report, with large banks producing thousands annually — creating quality inconsistencies and regulatory risk.

We present an AI-powered SAR Narrative Generator that cuts drafting time by up to 95% while maintaining full auditability. The system uses a Multi-Agent RAG pipeline built on Llama 3.1 8B (local LLM), LangChain, and ChromaDB. Four specialized agents — Data Analyst, Compliance Mapper, Narrative Writer, and QA Validator — collaboratively produce FinCEN-compliant narratives following the 5Ws+How structure.

Key innovations: (1) a complete audit trail tracing every AI decision to source data and regulatory references, (2) an XGBoost typology classifier trained on the SAML-D dataset for auto-detection of structuring, layering, and smurfing, and (3) a human-in-the-loop editor for analyst review and approval. The entire stack is open-source and runs on-premise, ensuring zero data exfiltration.
...
## 7. Working Prototype Screenshots

The following screenshots demonstrate the fully functional prototype:

### 7.1 Main Dashboard
![Main Dashboard — SAR Narrative Generator home page with metric cards, data upload, active cases, and system analytics](screenshots/01_main_dashboard.png)

### 7.2 Case Dashboard
![Case Dashboard — tabular view of all cases with risk levels, status breakdown, and processing metrics (45 sec avg generation, 93/100 quality)](screenshots/02_dashboard.png)

### 7.3 SAR Narrative Editor
![SAR Editor — case-level editor with quality scores (93 Overall, 95% Completeness, 98% Compliance), customer profile, ML typology prediction (SHAP), and generated narrative](screenshots/03_sar_editor.png)

### 7.4 Audit Trail Viewer
![Audit Trail Viewer — step-by-step reasoning trace showing Data Analyst, Compliance Mapper, RAG Retriever, Narrator, and QA Validator agents with data points and confidence scores](screenshots/04_audit_trail.png)

---

## 8. Sample SAR Narrative Output

Below is an actual output generated by our system for the demo scenario (CASE-7A3F21):

### INTRODUCTION

This Suspicious Activity Report (SAR) is being filed by HDFC Bank Ltd. to report suspicious transaction activity involving account holder Rajesh Kumar, account number XXXX-4521. The activity, observed between January 15–22, 2026, is consistent with structuring/smurfing patterns involving the receipt of funds from multiple unrelated sources followed by immediate international transfer. This is the initial SAR filing on this subject. Internal Investigation Reference: CASE-7A3F21.

### BODY (5Ws + How)

**WHO:** The subject, Rajesh Kumar (PAN: XXXXX1234K), has maintained a Current Account (XXXX-4521) with HDFC Bank, Andheri West Branch, Mumbai, since March 2019. KYC verification status: Verified (last updated: September 2025). The subject's declared occupation is Textile Export with a registered firm "Kumar Textiles" (GST: 27XXXXX1234K1ZF). Historical average monthly transaction volume: ₹3,00,000.
...
- Monthly volume spike: 16.7× above the declared average (₹50.12L vs. ₹3L/month)
- 20 unique senders with no prior transaction history with the subject
- No corresponding trade documentation or invoices
- Immediate consolidation and international transfer within 48 hours
- Pattern matches known structuring/smurfing typology

**HOW:** Funds were received via NEFT (17 transfers) and RTGS (3 transfers) from 20 distinct accounts held at 14 banks. After consolidation, ₹48,00,000 was transferred internationally via SWIFT. The remaining ₹2,12,000 was retained in the account.

### CONCLUSION

Based on the analysis, HDFC Bank Ltd. has determined that the transaction activity warrants filing of this SAR with FIU-IND. Enhanced monitoring, international transfer hold, and cross-referencing of all 20 sender accounts have been initiated.

> **Quality Scores:** Overall 93/100 | Completeness 95% | Compliance 98% | Readability 88% | Evidence Linkage 91%
> **Typology:** Structuring/Smurfing (Confidence: 94%) | **Generation Time:** ~45 seconds
